...
preposicoes = []
for s in txtB:
  if s[0] in zumbi and s[-1] not in zumbi:
    preposicoes.append (s)
print ('Questão A: {}'.format(len(preposicoes)))

#Questão B

verbos = []
for s in txtB:
  if len(s) == 7 and s[-1] not in zumbi:
    verbos.append (s)
print ('Questão B: {}'.format(len(verbos)))

#Questão C

pessoa = []
for s in txtB:
  if len(s) == 7 and s[-1] not in zumbi and s[0] not in zumbi:
    pessoa.append (s)
print ('Questão C: {}'.format(len(pessoa)))
...
for x in listaB:
  txt = ''
  for v in x:
    txt = txt + ordem [ord(v) - 65]
  lista2B.append(txt)
print ('Questão D: {}'.format(lista2B))
...
B = []
cont = 0
print ('Exercício E: os números mágicos são = ', end = ' ')
for x in txtB:
  n = z(x)
